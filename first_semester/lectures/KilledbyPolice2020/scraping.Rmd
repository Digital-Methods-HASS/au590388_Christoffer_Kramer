...
# scrape the website
url_html <- read_html(url) #Lader dig se en webside
```
...
whole_table <- url_html %>% #Scraper en side
 html_nodes("tr") %>% #Lader dig diktere hvor den skal scrape, scraper rækkerne. 
...
```{r}
class(mastertable) #Tjek hvilken datatype det er
...
data <- data %>% 
	mutate(Date =
			case_when(
				grepl("201[34]",Date) ~ mdy(Date),  #Tag fra kolonnen Date, og vælg alle rækker, der indeholder 2013 og 2014
