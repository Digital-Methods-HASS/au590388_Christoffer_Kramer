...
*License: I contacted The Commission on Presidential Debates and asked for permission to use their transcripts. They informed me, that the transcripts are in the public domain. 
...
save_plot_pdf("fig_1")
```
Hm.. interestingly enough one of the Democrat's primary talking points since 2008 appears to have been the opposite candidate, except for 2020. Is there also the same tendency by republicans?
...
```
Even when combining all years, some of the most mentioned word for republicans are "Obama" and "Clinton", while Democrats have talked a lot about "Mccain", "Donald", and "Bush". These few candidates appear so often, that otherwise common themes, such as national debt, jobs or budgets, are pushed aside. 
...
word_count <- left_join(word_count_party, total_words_party)

word_tf_idf <- word_count_party %>% 
...
save_plot_pdf("fig_6")
```
Here we also how fundamentally different. Rhetoric they use
...
```{r bar chart D, fig.cap="Fig. 12: Sentiment distribution since 1992 for Republicans - AFINN"}
sentiment_debate_words %>%
  filter(party == "D") %>% 
  filter(year >= 1992) %>% 
  count(value) %>%
...
Is there a correlation between when they were in power and how positive they were? Remember to filter out words like "vice". 
...
Lastly, let's get an overview by 
...
```{r commonality cloud, fig.cap="Fig. 3: Commonality cloud generated by stop word filtering"}
# stop_words_removed %>% 
#   filter(grepl("[DR]", party)) %>% 
#   count(party, word, sort = TRUE) %>% 
#   acast(word ~ party, value.var = "n", fill = 0) %>% 
...
#                    random.order = FALSE, 
#                    scale = c(2, 0.2)) +
#   title(main = "Fig. 3: Commonality Cloud - Both Parties")
# 
# save_plot_pdf("fig_3")
```